...
import { fetchSoldGalleryItems, fetchSoldProducts } from '../lib/api';
import { GallerySoldItem, Product } from '../lib/types';
import { useGalleryImages } from '../lib/hooks/useGalleryImages';

export function GalleryPage() {
  const [soldProducts, setSoldProducts] = useState<Product[]>([]);
  const [isLoadingSold, setIsLoadingSold] = useState(true);
  const [soldGalleryItems, setSoldGalleryItems] = useState<GallerySoldItem[]>([]);
  const [isLoadingSoldGallery, setIsLoadingSoldGallery] = useState(true);
  ...
  useEffect(() => {
    const loadSold = async () => {
      try {
        const [sold, soldGallery] = await Promise.all([
          fetchSoldProducts().catch((err) => {
            console.error('Error loading sold products:', err);
            return [];
          }),
          fetchSoldGalleryItems().catch((err) => {
            console.error('Error loading sold gallery items:', err);
            return [];
          }),
        ]);
        setSoldProducts(sold);
        setSoldGalleryItems(soldGallery);
      } catch (error) {
        console.error('Error loading gallery data:', error);
      } finally {
        setIsLoadingSold(false);
        setIsLoadingSoldGallery(false);
      }
    };
    loadSold();
  }, []);

  const isLoading = isLoadingGallery || isLoadingSold || isLoadingSoldGallery;
  const soldEntries = [
    ...
    ...soldGalleryItems.map((item) => ({
      id: `custom-${item.id}`,
      imageUrl: item.imageUrl,
      title: item.title || 'Custom Order',
                    ...
                    <div key={item.id} className="group bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <div className="p-3">
                        <div className="flex items-center justify-between gap-3 mb-1">
                          <h3 className="text-sm font-serif font-medium text-slate-900 truncate">{item.title}</h3>
                          <span className="text-sm font-serif font-medium text-slate-800 whitespace-nowrap">{item.badge}</span>
                        </div>
                        {item.subtitle && (
